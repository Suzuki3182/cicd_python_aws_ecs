...
resource "aws_security_group" "rds" {
  ...
  # Permite acesso apenas dos nós do EKS (porta 5432 PostgreSQL)
  ingress {
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [var.eks_security_group_id]
    description     = "Acesso dos nós EKS ao banco de dados"
  }

  # Permite acesso do Lambda (Lambda em VPC)
  ingress {
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
    cidr_blocks = ["10.0.0.0/16"] # CIDR da VPC - ajuste conforme necessário
    description = "Acesso interno da VPC"
  }
